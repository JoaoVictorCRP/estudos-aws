...
- Permite gerenciar os acesso às instâncias RDS por meio do IAM ao invés das credenciais tradicionais.

- Método seguro, garante o acesso com credenciais de curta-duração e controle de acesso centralizado.

- Possui suporte a auditoria via [CloudTrail](../seguranca/CloudTrail.md).

- **Disponível para instâncias MySQL, Postgres e MariaDB (e também as versões Aurora de MySQL e Postgres)**.
...
## Anotações
- O RDS roda em um servidor, no entanto você não consegue entrar no SO delas, somente no serviço de BD.
...
- Se você estiver utilizando uma RDS single-AZ e precisar fazer um backup ou snapshot, as operações de I/O serão suspensas.